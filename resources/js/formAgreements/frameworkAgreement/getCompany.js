...
function setVal(key, value) {
  let el = document.getElementById(key) || document.querySelector(`[name="${cssEscape(key)}"]`);
  if (!el) return false;
  el.value = value ?? "";
  return true;
}
...
function clearCompanyFields() {
  [
    ...
    "empresa_calle","empresa_numero","nro_calle","empresa_codigo_postal",
...
async function loadCompany(companyId) {
  try {
    ...
    setValAny(["empresa_codigo_postal","codigo_postal","cp"], dir.codigo_postal ?? dir.zip ?? "");
    ...
    const provName = company?.province_name ?? company?.direccion?.provincia ?? "";
...
async function loadEmployees(companyId) {
  ...
  try {
    ...
    if (Array.isArray(employees) && employees.length) {
      ...
      if (selectEmployeeContact) selectEmployeeContact.innerHTML =
        '<option value="">No hay representantes para esta empresa</option>';
      if (selectEmployeeFirma) selectEmployeeFirma.innerHTML =
        '<option value="">No hay representantes para esta empresa</option>';
    }
  } catch (error) {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  selectCompany.addEventListener("change", function () {
    ...
    if (!companyId) { clearCompanyFields(); clearEmployees(); return; }
